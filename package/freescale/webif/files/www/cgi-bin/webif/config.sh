...
case "$FORM_mode" in
	nochange) header "$FORM_cat" "" "@TR<<No config change.|No configuration changes were made.>>"
		echo "${FORM_prev:+<meta http-equiv=\"refresh\" content=\"3; URL=$FORM_prev\" />}" ;;
	clear)
		rm -r /tmp/.webif/* >&- 2>&-
		rm /tmp/.uci/* >&- 2>&-
	...
	review)
		...
		EDITED_FILES="${EDITED_FILES:+<h3 class="apply" style="display:inline">Edited files: </h3>$EDITED_FILES<br />}"
		echo "$EDITED_FILES"
		;;
	save)
		...
		echo "</pre></div>${FORM_prev:+<meta http-equiv=\"refresh\" content=\"4; URL=$FORM_prev\" />}"
		;;
esac
