...
$1 ~ /^newfield/ {
	...
	if ($4 == "hidden") field_opts = field_opts " style=\"display: none\""
	print "<tr" field_opts ">"
	if ($2 != "") print "<td width=\"60%\" >" $2 "</td><td width=\"40%\">"
	else print "<td colspan=\"2\">"
...
$1 ~ /^checkbox/ {
	...
	if (onchange != "") { 
           matched = index(onchange, "(")
	   if (matched) opts = opts " onchange=\"" onchange "\""
	   else opts = opts " onchange=\"" onchange "(this)\""
        }
	if (onclick != "") { 
	   matched = index(onclick, "(")
	   if (matched) opts = opts " onclick=\"" onclick "\""
	   else opts = opts " onclick=\"" onclick "(this)\""
	}
...
$1 ~ /^radio/ {
	...
	print "<input id=\"" $2 "\" type=\"radio\" name=\"" $2 "\" value=\"" $4 "\" " opts " />"
}
$1 ~ /^select/ {
	opts = ""
	if (onchange != "") { 
...
$1 ~ /^text$/ { 	
	...
	if ($6 == "readonly")
        print cols "<input id=\"" $2 "\" type=\"text\" name=\"" $2 "\" value=\"" $3 "\"  readonly=\"" $6 "\"/>" $4 cols_end
        else
...
$1 ~ /^submit/ { print "<input type=\"submit\" class=\"button\" name=\"" $2 "\" value=\"" $3 "\" />" }
$1 ~ /^reset/ { print "<input type=\"reset\" class=\"button\" name=\"" $2 "\" value=\"" $3 "\" />" }
