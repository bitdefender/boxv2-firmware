...
ifeq ($(findstring linaro, $(CONFIG_GCC_VERSION)),linaro)
    ...
    ifeq ($(CONFIG_GCC_VERSION),"6.2-linaro")
      PKG_REV:=6.2-2016.11
      PKG_VERSION:=6.2.1
      PKG_VERSION_MAJOR:=6.2
      PKG_MD5SUM:=a304fcc009333b9594d8790940656edd
      PKG_COMP:=xz
      LINARO_RELEASE=6.2.1
    endif
    ifeq ($(CONFIG_GCC_VERSION),"6.3-linaro")
      PKG_REV:=6.3-2017.02
      PKG_VERSION:=6.3.1
      PKG_VERSION_MAJOR:=6.3
      PKG_MD5SUM:=563d2332bbe9928e841da66ffefa8fbf
      PKG_COMP:=xz
      LINARO_RELEASE=6.3.1
    endif
    ifeq ($(CONFIG_GCC_VERSION),"7.3-linaro")
      PKG_REV:=7.3-2018.04
      PKG_VERSION:=7.3.1
      PKG_VERSION_MAJOR:=7.3
      PKG_MD5SUM:=a86cce28a4f82b42f94318528b01aa21
      PKG_COMP:=xz
      LINARO_SNAPSHOT=7.3.1
    endif
    ifneq ($(LINARO_RELEASE),)
      PKG_SOURCE_URL:=https://releases.linaro.org/components/toolchain/gcc-linaro/$(PKG_REV)
    else
        ifneq ($(LINARO_SNAPSHOT),)
            PKG_SOURCE_URL:=https://snapshots.linaro.org/components/toolchain/gcc-linaro/$(PKG_REV)
        else
    ...
    ifneq ($(LINARO_SNAPSHOT),)
        PKG_SOURCE:=$(PKG_NAME)-linaro-snapshot-$(PKG_REV).tar.$(PKG_COMP)
        GCC_DIR:=gcc-linaro-snapshot-$(PKG_REV)
    else
